...
import { useGetCustomerInfos } from "../hooks/useGetCustomerInfos";
import {
  Alert,
  Avatar,
  Card,
...
export const AccountInformations = () => {
  ...
  const [getCustomerInfos, responseData, isLoading, isError] =
    useGetCustomerInfos(customerID);
  ...
  const createData = (id: number, amount: number) => {
    return { id, amount };
  };

  const rows = responseData
    ? responseData.transactions.map((item: { id: number; amount: number }) =>
        createData(item.id, item.amount)
      )
    : [];
        ...
        >
          ...
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              ...
              <TableBody>
                {rows.map((row: { id: number; amount: number }) => (
                  <TableRow
                    key={row.id}
